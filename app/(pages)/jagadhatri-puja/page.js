                ...
                <div className="flex flex-col gap-6 text-justify">
                    ...
                    <div className="flex items-center justify-center">
                        <Link href={`/jagadhatri-puja/${uptoYear}`}
                              className="btn border-2 uppercase py-3.5 px-6 h-auto min-h-full rounded-md bg-transparent border-yellow-500 text-yellow-500 hover:bg-transparent hover:border-yellow-500 hover:text-yellow-500">View
                            Jagadhatri
                            Puja {uptoYear} Details</Link>
                    </div>
                    ...
                    <div className="grid grid-cols-3 md:grid-cols-6 xl:grid-cols-12 gap-4">
                        {yearsArray.map((year, index) => (
                            <Link key={index} href={`/jagadhatri-puja/${year}`}
                                  className="bg-gray-50 hover:bg-gray-100 p-3 text-center border rounded-md border-neutral-200">{year}</Link>
                        ))}
                    </div>
                </div>
            </Section>
